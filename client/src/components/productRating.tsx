...
export const ProductRating: FC<{ num: number }> = ({ num }): ReactElement | null => {
  ...
  const setFilledStars = (_: number, index: number): ReactElement => (
    <SVGElement key={index} ariaHidden={true} className='w-5 h-5 lg:w-6 lg:h-6 fill-yellow-400 hover:fill-yellow-300'>
  ...
  const setUnfilledStars = (_: number, index: number): ReactElement => (
    <SVGElement key={ratingNum + index} ariaHidden={true} className='w-5 h-5 lg:w-6 lg:h-6 fill-gray-300 transform hover:rotate-6'>
