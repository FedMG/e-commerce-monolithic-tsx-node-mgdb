...
export const HeaderMenuList: React.FC<HeaderMenuListProps> = ({
  links,
  bgColor,
  selectOption,
  spacing
      ...
      {links.map(({ name, path, dropdown }) => {
        if (dropdown != null) {
          return (
            <List.Item
              ...
              key={name}
              role='menuitem'
              className={`w-full ${isString(bgColor) ? (bgColor as string) : ''}`}>
              <HeaderDropdown label={name} links={dropdown} selectOption={selectOption} WithMenuList={HeaderMenuList} />
            </List.Item>
          )
        }
        ...
        return (
          <List.ItemEvent
            key={name}
            role='menuitem'
            className='w-full bg-inherit hover:bg-gray-300 active:bg-gray-200 lg:rounded-md'
            ...
            >
              <Text
                className={`${
                  spacing === true ? 'pl-4 lg:pl-2 border-l border-gray-300 lg:border-none' : ''
                } py-1 h-full`}>
                {' '}
                {name}
              </Text>
